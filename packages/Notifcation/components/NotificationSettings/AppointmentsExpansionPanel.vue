    ...
    <v-card>
      ...
      <v-list tow-line>
        <v-list-item class="mb-14" v-for="(item,n) in settingsTitle "  :key="n">
          ...
          <v-list-item-content>
            <v-spacer></v-spacer>
            <v-list-item-subtitle v-if="n == 0"> Notify me when: </v-list-item-subtitle>
            ...
            <v-switch
              ...
              hide-details
              :checked="item.active"
              @click="appointmentSettings(item ,n)"
              inset 
            >
             <template #prepend>
    <v-list-item-subtitle class="small" v-if="item.active">On</v-list-item-subtitle>
    <v-list-item-subtitle class="small" v-if="!item.active">Off</v-list-item-subtitle>
  </template>
            </v-switch>
          </v-list-item-content>
...
export default {
  name: "AppointmentsExpansionPanel",
  data: () => ({
    ...
    settingsTitle:[
        ...
        },
        {
            ...
            title :"Note added on appointment:",
            active: false,
            reminderAfterOneHour:false
  ...
  methods: {
    resetSittings() {},
    appointmentSettings(item,id){
       console.log('active here',item.active);
       console.log('reminder here',item.reminderAfterOneHour);
       if (!this.appointmentSettingsValues.includes(item)) {
           
           this.appointmentSettingsValues.push(item)
          
       } if(item.reminderAfterOneHour) {

            this.appointmentSettingsValues[id].reminderAfterOneHour =item.reminderAfterOneHour  
       }
        // here create the request 
       console.log('this is the array',this.appointmentSettingsValues);
...
<style  scoped>
.v-input .ml-3 .v-input--hide-details .v-input--is-label-active .v-input--is-dirty .theme--light .v-input--selection-controls .v-input--switch .v-input--switch--inset .info--text:after {
    content: "No";
    color: #333333;
    font-size: 10px;
}
