        ...
        <v-list-item class="mb-14" v-for="(item, n) in settingsTitle" :key="n" :v-if="!movedSettingsArray.includes(item)">
          ...
          <v-list-item-content>
            ...
            <v-switch
              ...
              hide-details
              :checked="item.active"
              @click="RemindersSettings(item, n)"
              inset
            >
              <template #prepend>
...
export default {
  name: "Reminders",
  data: () => ({
    RemindersSettingsValues: [],
    movedSettings: [],
//TODO: change this to state  for all title settings 

    settingsTitle: [
      {
        title: "Remind me before 5 minutes of any meeting:",
        active: false,
  ...
  methods: {
    ...
    RemindersSettings(item, id) {
      console.log("active here", item.active);
      console.log("reminder here", item.reminderAfterOneHour);
      if (!this.RemindersSettingsValues.includes(item)) {
        this.RemindersSettingsValues.push(item);
      }
      if (item.reminderAfterOneHour) {
        this.RemindersSettingsValues[id].reminderAfterOneHour =
          item.reminderAfterOneHour;
      }
      // here create the request
      console.log("this is the array", this.RemindersSettingsValues);
