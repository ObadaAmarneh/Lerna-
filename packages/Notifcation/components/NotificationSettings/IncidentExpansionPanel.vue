        ...
        <v-list-item class="mb-14" v-for="(item, n) in settingsTitle" :key="n">
          <v-list-item-content>
            ...
            <v-list-item-action-text
              ><strong>*</strong> Assigned to me</v-list-item-action-text
            >
          </v-list-item-content>
          <v-list-item-content>
            <v-spacer></v-spacer>
            ...
            <v-switch
              ...
              hide-details
              :checked="item.active"
              @click="incidentSettings(item, n)"
              inset
              :disabled="n > 0 && !settingsTitle[0].active"
            >
              <template #prepend>
...
export default {
  ...
  methods: {
    ...
    incidentSettings(item, id) {
      console.log("active here", item.active);
      console.log("reminder here", item.reminderAfterOneHour);
      if (!this.incidentSettingsValues.includes(item)) {
        this.incidentSettingsValues.push(item);
      }
      if (item.reminderAfterOneHour) {
        this.incidentSettingsValues[id].reminderAfterOneHour =
          item.reminderAfterOneHour;
      }
      // here create the request
      console.log("this is the array", this.incidentSettingsValues);
