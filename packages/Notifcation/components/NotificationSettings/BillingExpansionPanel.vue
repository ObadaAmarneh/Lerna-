...
//TODO: change this to state  for all title settings 
  ...
  methods: {
    ...
    billingSettings(item, id) {
      ...
      if (!this.billingSettingsValues.includes(item)) {
        this.billingSettingsValues.push(item);
      }
      if (item.reminderAfterOneHour) {
        this.billingSettingsValues[id].reminderAfterOneHour =
          item.reminderAfterOneHour;
      }
      // here create the request
      console.log("this is the array", this.billingSettingsValues);
