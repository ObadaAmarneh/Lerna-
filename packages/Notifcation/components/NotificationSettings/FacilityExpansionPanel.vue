...
export default {
  name: "FacilityExpansionPanel",
  data: () => ({
    facilitySettingsValues: [],
    movedSettings: [],
//TODO: change this to state  for all title settings 

    settingsTitle: [
      {
        title: "Machines broke down:",
        active: false,
  ...
  methods: {
    ...
    facilitySettings(item, id) {
      console.log("active here", item.active);
      console.log("reminder here", item.reminderAfterOneHour);
      if (!this.facilitySettingsValues.includes(item)) {
        this.facilitySettingsValues.push(item);
      }
      if (item.reminderAfterOneHour) {
        this.facilitySettingsValues[id].reminderAfterOneHour =
          item.reminderAfterOneHour;
      }
      // here create the request
      console.log("this is the array", this.facilitySettingsValues);
