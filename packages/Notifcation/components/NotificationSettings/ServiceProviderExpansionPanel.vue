<template>
  <div>
    <v-card>
      ...
      <v-list tow-line>
        <v-list-item class="mb-14" v-for="(item, n) in settingsTitle" :key="n" :v-if="!movedSettingsArray.includes(item)">
          <v-list-item-content>
            {{ item.title }}
          ...
          <v-list-item-content>
            ...
            <v-switch
              ...
              hide-details
              :checked="item.active"
              @click="ServiceProviderSettings(item, n)"
              inset
            >
              <template #prepend>
...
export default {
  name: "ServiceProviderExpansionPanel",
  data: () => ({
    ServiceProviderSettingsValues: [],
    movedSettings: [],
//TODO: change this to state  for all title settings 

    settingsTitle: [
      {
        title: "Service Provider profile incomplete:",
        active: false,
      ...
      {
        title: "Service Provider profile deleted:",
        active: false,
        reminderAfterOneHour: false,
      },
    ],
  ...
  methods: {
    ...
    ServiceProviderSettings(item, id) {
      console.log("active here", item.active);
      console.log("reminder here", item.reminderAfterOneHour);
      if (!this.ServiceProviderSettingsValues.includes(item)) {
        this.ServiceProviderSettingsValues.push(item);
      }
      if (item.reminderAfterOneHour) {
        this.ServiceProviderSettingsValues[id].reminderAfterOneHour =
          item.reminderAfterOneHour;
      }
      // here create the request
      console.log("this is the array", this.ServiceProviderSettingsValues);
  ...
  computed: {
    movedSettingsArray() {
      return this.$store.getters.getMovedSettings;
    },
  },
};
</script>
